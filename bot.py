    ...
    try:
        ...
        if chat_id != ADMIN_ID and auto_delete:
            ...
            pending.append({
                ...
                "delete_at": time.time() + settings["preview_delete_delay"],
            })
...
def save_preview_admin_activity(data: dict):
    try:
        with open(PREVIEW_ADMIN_ACTIVITY_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.error(f"Preview admin activity write error: {e}")


async def create_preview_admin_activity(
    bot,
    chat_id: int,
    full_name: str,
    username: str,
    ...
    """Create one persistent admin notification for Preview A.

    The same admin message is later edited to 'Album Dihapus'.  The
    mapping is persisted so a bot/VPS restart cannot orphan the activity.
    """
    if chat_id == ADMIN_ID:
        return
    ...
    try:
        ...
        logger.error(f"Failed to create Preview A admin activity: {e}")
        return

    activities = read_preview_admin_activity()
    activity_key = str(message.message_id)
    ...
    save_preview_admin_activity(activities)
...
async def mark_preview_admin_activity_deleted(
    ...
    reason: str = "auto_delete",
):
    """Edit the exact Preview A admin notification for one delivery.

    The lookup is based on the delivered message IDs, not only chat_id, so
    a new Preview A delivery can never be incorrectly marked deleted by an
    older timer that wakes up later.
    """
    if chat_id == ADMIN_ID:
        return

    activities = read_preview_admin_activity()
    target_ids = list(message_ids or [])
    activity = None

    for key, candidate in activities.items():
        if candidate.get("chat_id") != chat_id:
            continue
        if candidate.get("status") == "DELETED":
            continue
        candidate_ids = list(candidate.get("media_message_ids") or [])
        if target_ids and candidate_ids == target_ids:
            activity = candidate
            break
    ...
    message_id = activity.get("admin_message_id")
    if not message_id:
        activity["status"] = "DELETED"
        activity["deleted_at"] = time.time()
        activity["delete_reason"] = reason
        save_preview_admin_activity(activities)
        return
    ...
    try:
        ...
        text = (
            f"📮 *Album Dihapus*\n\n"
            f"👤 Name: {activity.get('full_name', '-') }\n"
            f"🔗 Username: {activity.get('username', '-') }\n"
            f"🆔 User ID: `{activity.get('user_id', '-')}`\n\n"
            f"Time: {now}"
        )
        ...
        # Keep the activity ACTIVE when the edit fails so a later trigger can
        # retry. Never persist DELETED unless the Telegram edit succeeded.
        logger.warning(
            f"Preview A admin activity edit failed chat_id={chat_id} "
            f"message_id={message_id} reason={reason}: {e}"
        )
        return
...
async def update_vip_activity(
    ...
    if "qris" in activity["steps"]:
        step_lines.append("👁️ 📋 Halaman Qris")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        if ok:
            ...
            increment_counter()
            if payload == DEEP_LINK_A:
                await create_preview_admin_activity(
                    context.bot,
                    update.effective_chat.id,
                    full_name,
                    username,
                    user_id,
                    last_delivered_messages.get(update.effective_chat.id, [])
                )
            else:
                await notify_admin(context.bot, full_name, username, user_id)
        ...
        if pending:
            ...
            if ok:
                ...
                await create_preview_admin_activity(
                    ...
                    username,
                    user_id,
                    last_delivered_messages.get(chat_id, [])
                )
...
async def sweep_pending_preview_deletions(bot):
    ...
    for entry in pending:
        ...
        if delete_at is not None:
            delay = max(0, float(delete_at) - time.time())
        else:
            # Backward-compatible recovery for old pending entries created
            # before persistent delete_at was introduced.
            delay = settings["preview_delete_delay"]
...
def main():
    ...
    async def start_background(app):
        ...
        # Recover Preview A deletion timers after a bot/VPS restart without
        # touching PREV2 or unrelated background workflows.
        try:
            settings = read_settings()
            if settings["preview_auto_delete"]:
                await sweep_pending_preview_deletions(app.bot)
        except Exception as e:
            logger.error(f"Preview auto-delete startup recovery error: {e}")
