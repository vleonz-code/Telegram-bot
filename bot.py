...
async def vipnav_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Race-guard: tandai tap ini sebagai yang paling baru untuk chat ini.
    # Operasi dict biasa (sinkron, tidak nunggu apa-apa) -- bukan lock.
    ...
    # Race-guard: kalau sudah ada tap lebih baru masuk sejak token ini
    # distempel, jangan kirim edit yang sudah basi sama sekali.
    if vip_nav_latest.get(_vip_nav_chat_id) != _vip_nav_token:
